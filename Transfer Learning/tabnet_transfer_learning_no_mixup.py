...
DESCRIPTOR = 'transfer-no-mixup'+'-'+datetime.now().strftime('%F_%H-%M-%S')
...
def run_training(params):
    ...
    for class_ in tqdm(['major', 'minor'], desc = f'Tuning For Major and Minor Classes - Fold {params["fold"]}'):

        # Fit the model with the major or minor classes
        model.fit(
            dset[class_]['train']['X'].values,
            dset[class_]['train']['y'].values,
            eval_set = [(dset[class_][c]['X'].values, dset[class_][c]['y'].values) for c in ['train', 'eval']], 
        ...
        model.save_model(os.path.join('Models', DESCRIPTOR+'_'+class_))

        # Freeze all but the last step
        if class_ == 'major':
            model.freeze_steps(params['n_steps'] - 1)
            assert np.any([layer.requires_grad for layer in model.network.tabnet.parameters()]), 'Freezing failed.'

        # Predict for all subsets
        for set_ in dset[class_]:
